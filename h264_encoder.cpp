...
static int get_v4l2_colorspace(std::optional<libcamera::ColorSpace> const &cs) {
    ...
    std::cout << "H264: surprising colour space: "
              << libcamera::ColorSpace::toString(cs) << std::endl;
    return V4L2_COLORSPACE_SMPTE170M;
}
...
H264Encoder::H264Encoder(StreamInfo const &info)
    ...
    if (fd_ < 0) throw std::runtime_error("failed to open V4L2 H264 encoder");
    std::cout << "Opened H264Encoder on " << device_name << " as fd " << fd_
              << std::endl;

    // Set the output and capture formats. We know exactly what they will be.
    std::cout << "info width: " << info.width
              << " infor height: " << info.height << std::endl;
    v4l2_format fmt = {};
    ...
    fmt.fmt.pix_mp.width = 640;   // options->width;
    fmt.fmt.pix_mp.height = 480;  // options->height;
    ...
    double frate = 30.0;  // this should be an option
    if (frate > 0.0) {
    ...
    // We have to maintain a list of the buffers we can use when our caller
    // gives us another frame to encode.
    ...
    for (unsigned int i = 0; i < reqbufs.count; i++) {
        ...
        if (xioctl(fd_, VIDIOC_QUERYBUF, &buffer) < 0)
            throw std::runtime_error("failed to capture query buffer " +
                                     std::to_string(i));
        buffers_[i].mem =
            mmap(0, buffer.m.planes[0].length, PROT_READ | PROT_WRITE,
                 MAP_SHARED, fd_, buffer.m.planes[0].m.mem_offset);
        ...
        // Whilst we're going through all the capture buffers, we may as well
        // queue them ready for the encoder to write into.
...
H264Encoder::~H264Encoder() {
    ...
    reqbufs.memory = V4L2_MEMORY_DMABUF;
    if (xioctl(fd_, VIDIOC_REQBUFS, &reqbufs) < 0) {
        std::cout << "Request to free output buffers failed" << std::endl;
    }
    ...
    reqbufs.memory = V4L2_MEMORY_MMAP;
    if (xioctl(fd_, VIDIOC_REQBUFS, &reqbufs) < 0) {
        std::cout << "Request to free capture buffers failed" << std::endl;
    }
...
void H264Encoder::EncodeBuffer(int fd, size_t size, void *mem,
                               StreamInfo const &info, int64_t timestamp_us) {
    std::cout << "encode buffer: size=" << size << std::endl;
    int index;
    {
        ...
        if (input_buffers_available_.empty())
            throw std::runtime_error(
                "no buffers available to queue codec input");
    ...
    std::cout << "buffer queued to encoder " << std::endl;
}
...
void H264Encoder::pollThread() {
    while (true) {
        ...
        std::cout << "polling return " << ret << std::endl;

        if (ret == -1) {
            if (errno == EINTR) {
                std::cout << "poll error EINTR, continue" << std::endl;
                continue;
            }
            throw std::runtime_error("unexpected errno " +
                                     std::to_string(errno) + " from poll");
        }
        if (p.revents & POLLIN) {
            v4l2_buffer buf = {};
            v4l2_plane planes[VIDEO_MAX_PLANES] = {};
            ...
            std::cout << "h264 encoder dq buffer " << buf.index << std::endl;
            if (ret == 0) {
                // Return this to the caller, first noting that this buffer,
                // identified by its index, is available for queueing up another
                // frame.
                {
                    std::lock_guard<std::mutex> lock(
                        input_buffers_available_mutex_);
                    input_buffers_available_.push(buf.index);
                    std::cout << "add buffer " << buf.index << " to available q"
                              << std::endl;
                }
            ...
            if (ret == 0) {
                ...
                int64_t timestamp_us =
                    (buf.timestamp.tv_sec * (int64_t)1000000) +
...
void H264Encoder::outputThread() {
    ...
    while (true) {
        ...
        output_ready_callback_(item.mem, item.bytes_used, item.timestamp_us,
                               item.keyframe);
        v4l2_buffer buf = {};
        v4l2_plane planes[VIDEO_MAX_PLANES] = {};
        buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE;
