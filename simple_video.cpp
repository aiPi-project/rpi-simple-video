...
const std::vector<const char *> heapNames{"/dev/dma_heap/vidbuf_cached",
...
void SimpleVideo::requestCompleted(Request *request) {
    ...
    for (auto const &buffer_map : request->buffers()) {
        ...
        if (it == mapped_buffers_.end()) {
            std::cout << "failed to identify request complete buffer"
                      << std::endl;
            return;
        }
        ...
        if (ret) {
            std::cout << "failed to sync dma buf on request complete"
                      << std::endl;
            return;
        }
        std::cout << "buffer stream: "
                  << buffer_map.first->configuration().toString() << std::endl;
    }
    // signal completed?
    std::cout << "request completed!!!" << std::endl;

    CompletedRequestPtr payload =
        std::make_shared<CompletedRequest>(sequence++, request);
...
bool SimpleVideo::QueueRequest(CompletedRequestPtr completed_request) {
    ...
    for (auto const &p : buffers) {
        ...
        int ret = ::ioctl(p.second->planes()[0].fd.get(), DMA_BUF_IOCTL_SYNC,
                          &dma_sync);
        if (ret) {
        ...
        if (request->addBuffer(p.first, p.second) < 0) {
            std::cout << "failed to add buffer to request in QueueRequest"
                      << std::endl;
            return false;
        }
    }
...
libcamera::UniqueFD SimpleVideo::DmaHeapAlloc(const char *name,
                                              std::size_t size) {
...
bool SimpleVideo::SetupCapture() {
    ...
    for (StreamConfiguration &config : *configuration_) {
        ...
        for (size_t i = 0; i < config.bufferCount; i++) {
            ...
            libcamera::UniqueFD fd =
                DmaHeapAlloc(name.c_str(), config.frameSize);
            if (!fd.isValid()) {
                std::cout << "failed to allocate capture buffer for stream"
                          << std::endl;
                return false;
            }
            ...
            mapped_buffers_[frame_bufs.back().get()].push_back(
                libcamera::Span<uint8_t>(static_cast<uint8_t *>(memory),
                                         config.frameSize));
        }
...
bool SimpleVideo::EncodeBuffer(CompletedRequestPtr &completed_request,
                               Stream *stream) {
