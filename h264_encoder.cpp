...
static int get_v4l2_colorspace(std::optional<libcamera::ColorSpace> const &cs)
{
	...
	std::cout << "H264: surprising colour space: " << libcamera::ColorSpace::toString(cs) << std::endl;
	return V4L2_COLORSPACE_SMPTE170M;
}
	...
	: abortPoll_(false), abortOutput_(false)
{
    ...
    std::cout << "info width: " << info.width << " infor height: " << info.height << std::endl;
	v4l2_format fmt = {};
	...
	fmt.fmt.pix_mp.width = 640; //1920;  // options->width;
	fmt.fmt.pix_mp.height = 480; //1080; // options->height;
	...
	double frate = 30.0; // options->framerate.value_or(DEFAULT_FRAMERATE);
	if (frate > 0.0)
	{
		...
		if (p.revents & POLLIN)
		{
			...
			if (ret == 0)
			{
				...
				// by its index, is available for queueing up another frame.
				{
                    ...
                    std::cout << "add buffer " << buf.index << " to available q" << std::endl;
				}
